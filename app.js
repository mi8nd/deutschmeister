...
const handleNavigation = (hash, updateHistory = true) => {
    ...
    if (viewId === 'video-player') {
        ...
        if (level && PLAYLISTS[level]) {
            currentLevel = level;
            if (document.getElementById('video-view-title')) document.getElementById('video-view-title').textContent = `${translations[localStorage.getItem('language') || 'en'].levelPrefix}${currentLevel} Course`;
...
const startQuiz = (level) => {
    ...
    if (document.getElementById('quiz-title')) document.getElementById('quiz-title').textContent = `${translations[localStorage.getItem('language') || 'en'].levelPrefix}${level} Quiz`;
    displayQuestion();
    showView('quiz');
};
...
const handleAnswer = (selectedIndex) => {
    ...
    if (selectedIndex === question.correctAnswer) {
        ...
        if (feedbackEl) feedbackEl.textContent = translations[lang].feedbackCorrect;
    } else {
        ...
        if (feedbackEl) feedbackEl.innerHTML = `${translations[lang].feedbackIncorrectPrefix}<strong>${correctAnswerText}</strong>. <br><em>${question.explanation || ''}</em>`;
    }
    setTimeout(() => {
...
const renderAllCourses = () => {
    ...
    Object.keys(PLAYLISTS).forEach((level) => {
        ...
        card.innerHTML = `
            ...
            <p class="course-description">${translations[lang].courseDescription}</p>
            <button class="btn btn-primary start-course-btn" data-level="${level}">${translations[lang].startCourse}</button>
        `;
...
const setupEventListeners = () => {
    ...
    document.body.addEventListener('click', async (e) => {
        ...
        if (target.closest('#install-app-btn')) {
            ...
            const lang = localStorage.getItem('language') || 'en';
            if (elements.resetCourseConfirmText) elements.resetCourseConfirmText.textContent = translations[lang].resetCourseConfirmMessage.replace('{level}', currentLevel);
