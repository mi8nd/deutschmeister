...
let currentPlaylist = [],
	...
	score = 0;
let player, lastSavedTimestamp = 0;
const elements = {};
let oobCode = null;
let allVideosData = {}; // This is now a local cache, populated on demand
...
const setLanguage = (lang) => {
	...
	if (currentLevel) {
		getPlaylistFromCache(currentLevel).then(playlist => {
			currentPlaylist = playlist;
			if (!document.getElementById('video-player-view').classList.contains('hidden')) {
				renderVideoList();
			}
		});
	}
...
const handleNavigation = (hash, updateHistory = true) => {
	...
	if (previousView && previousView.id === 'video-player-view') {
		if (player && typeof player.getCurrentTime === 'function') {
			saveTimestamp(player.getVideoData().video_id, player.getCurrentTime());
		}
...
const getTranslatedVideoInfo = (videoId) => {
	const rawVideoInfo = allVideosData[videoId];
	if (!rawVideoInfo) return null;

	const lang = localStorage.getItem('language') || 'en';
	const translatedTitles = translations[lang]?.videoTitles?.[rawVideoInfo.level] || {};
	const translatedTitle = translatedTitles[rawVideoInfo.videoId] || rawVideoInfo.title;
...
const getPlaylistFromCache = async (level) => {
	const lang = localStorage.getItem('language') || 'en';
	const videosForLevel = await getAndCacheVideosForLevel(level);

	// Update the global cache
	videosForLevel.forEach(video => {
		if (!allVideosData[video.videoId]) {
			allVideosData[video.videoId] = video;
		}
	});

	const translatedTitles = translations[lang]?.videoTitles?.[level] || {};

	return videosForLevel.map(video => ({
		...video,
		title: translatedTitles[video.videoId] || video.title,
	}));
};
...
// Efficient Profile Picture Upload using Firebase Storage
...
const startQuiz = (level) => {
	...
	if (!currentQuiz.length) {
		showToast("No quiz for this level yet.", "info");
		return;
	}
	currentQuestionIndex = 0;
	score = 0;
...
const handleAnswer = (selectedIndex) => {
	...
	if (selectedIndex === question.correctAnswer) {
		...
		const correctAnswerText = (question.options[question.correctAnswer] === "True") ?
			translations[lang].trueOption : (question.options[question.correctAnswer] === "False" ?
				translations[lang].falseOption : question.options[question.correctAnswer]);
...
const saveTimestamp = async (videoId, time) => {
	if (!currentUser || !videoId || time < 1 || Math.abs(time - lastSavedTimestamp) < 5) return;
	lastSavedTimestamp = time;
...
const onPlayerStateChange = (event) => {
	if (event.data === YT.PlayerState.PAUSED) {
		saveTimestamp(event.target.getVideoData().video_id, event.target.getCurrentTime());
	}
};
...
const markVideoAsComplete = async (videoIndex) => {
	...
	await updateDoc(doc(db, "users", currentUser.uid), {
		[`progress.${currentLevel}`]: newProgress
	});
};
...
const resetSingleVideoProgress = async (videoId) => {
	...
	if (userProgress.timestamps?.[videoId] !== undefined) {
		...
		await updateDoc(userDocRef, {
			[`timestamps.${videoId}`]: deleteField()
		});
...
const renderContinueWatching = () => {
	...
	// Use the local cache `allVideosData` to find video info
	for (const videoId in timestamps) {
		if (timestamps[videoId] < 5) continue;
		const videoInfo = getTranslatedVideoInfo(videoId);
	...
	recentVideos.forEach(video => {
		...
		const progressPercent = video.durationInSeconds > 0 ? Math.min(100, (video.timestamp / video.durationInSeconds) * 100) : 0;
		card.innerHTML = `
...
const cacheDOMElements = () => {
	const ids = ['app-loader', 'auth-container', 'app-container', 'toast-container', 'login-form', 'signup-form', 'dark-mode-toggle', 'logout-modal-overlay', 'confirm-logout-btn', 'cancel-logout-btn', 'reset-progress-checkbox', 'reset-course-modal-overlay', 'reset-course-confirm-text', 'confirm-reset-btn', 'cancel-reset-btn', 'youtube-player-container', 'video-list', 'welcome-message', 'profile-name', 'profile-email', 'delete-account-modal-overlay', 'cancel-delete-btn', 'confirm-delete-btn', 'password-reset-view', 'password-reset-form', 'change-password-modal-overlay', 'change-password-form', 'cancel-change-password-btn', 'change-password-error', 'sidebar', 'hamburger-btn', 'close-sidebar-btn', 'install-app-btn', 'reset-all-modal-overlay', 'cancel-reset-all-btn', 'confirm-reset-all-btn', 'pfp-upload-input', 'pfp-container', 'faq-view', 'terms-view', 'privacy-view', 'accessibility-view', 'lang-toggle-btn', 'lang-dropdown', 'forgot-password-modal-overlay', 'forgot-password-form-modal', 'cancel-forgot-password-btn', 'forgot-password-error', 'video-player-view'];
	ids.forEach(id => {
...
const setupEventListeners = () => {
	...
	// --- Specific Container Event Listeners for Maintainability ---

	// Authentication Container
	elements.authContainer?.addEventListener('click', async (e) => {
		...
		if (target.id === 'forgot-password-link') {
			...
			elements.forgotPasswordFormModal.reset();
			elements.forgotPasswordModalOverlay.classList.remove('hidden');
		}

		if (socialButton) {
			const provider = socialButton.dataset.provider;
			if (provider === 'google') {
				const isSignUpFlow = socialButton.closest('.form-container').classList.contains('sign-up-container');
				const result = await handleSignInWithGoogle(isSignUpFlow);
				if (!result.success) {
					if (result.error === 'NO_ACCOUNT_FOUND') {
						showToast(translations[lang].noAccountFoundGoogle, 'error');
					} else if (result.error && !result.error.includes('popup-closed-by-user')) {
	...
	// Main App Container (for delegated clicks)
	elements.appContainer?.addEventListener('click', async (e) => {
		const target = e.target;
		const lang = localStorage.getItem('language') || 'en';
		...
		if (continueCard || target.matches('.start-course-btn')) {
			currentLevel = target.closest('[data-level]').dataset.level;
			document.getElementById('video-view-title').textContent = `${translations[lang].levelPrefix}${currentLevel} Course`;
			showView('video-player');
			elements.videoList.innerHTML = '<div class="spinner"></div>';
			const playlist = await getPlaylistFromCache(currentLevel);
			currentPlaylist = playlist;
			if (playlist.length) {
				renderVideoList();
				const videoIdToLoad = continueCard ? continueCard.dataset.videoId : playlist[0].videoId;
		...
		// Quiz Actions
		if (target.matches('.start-quiz-btn')) startQuiz(target.dataset.level);
		...
		// Modal Triggers
		if (target.matches('#logout-btn')) elements.logoutModalOverlay.classList.remove('hidden');
		...
		if (navLink || (footerLink && footerLink.getAttribute('href').startsWith('#'))) {
			e.preventDefault();
			const link = navLink || footerLink;
			handleNavigation(link.getAttribute('href'));
			if (window.innerWidth <= 992) closeMobileMenu();
		}
		if (target.closest('#install-app-btn') && deferredInstallPrompt) {
	...
	document.body.addEventListener('click', (e) => {
		const target = e.target;
		if (document.body.classList.contains('sidebar-open') && !target.closest('.sidebar')) closeMobileMenu();
		if (!target.closest('.lang-toggle-container')) elements.langDropdown?.classList.add('hidden');
	...
	elements.loginForm?.addEventListener('submit', async (e) => {
		...
		if (!result.success) loginError.textContent = translations[lang].invalidCredentials;
	});
	elements.signupForm?.addEventListener('submit', async (e) => {
		...
		if (!result.success) signupError.textContent = result.error.includes('auth/email-already-in-use') ? translations[lang].emailInUse : translations[lang].signupError;
	});
	elements.forgotPasswordFormModal?.addEventListener('submit', async (e) => {
		e.preventDefault();
		...
		if (result.success) {
			showToast(translations[lang].resetEmailSent, 'success');
			elements.forgotPasswordModalOverlay.classList.add('hidden');
		} else {
			errorEl.textContent = translations[lang].resetEmailFailed;
		}
	});
	...
	document.querySelectorAll('.modal-overlay').forEach(overlay => {
		overlay.addEventListener('click', (e) => {
			if (e.target === overlay || e.target.id.startsWith('cancel-')) {
	...
	elements.confirmResetAllBtn?.addEventListener('click', () => {
		resetAllProgress();
...
const initializeApp = async () => {
	try {
		// Only fetch video counts on initial load for performance.
		courseData = await fetchPlaylistVideoCounts();
		if (!courseData) {
			throw new Error("Course data could not be fetched.");
		}

		onAuthStateChanged(auth, async (user) => {
			if (oobCode) return; // Wait for password reset flow to complete
			if (user) {
				const userDoc = await getDoc(doc(db, "users", user.uid));
				if (userDoc.exists()) {
					updateUIForUser(user, userDoc.data());
				} else {
					console.warn("User exists in Auth but not Firestore. Logging out.");
					await handleLogout();
				}
			} else {
				updateUIForGuest();
		...
		showToast('Critical Error: Could not load app data. Please refresh.', 'error');
		elements.appLoader.innerHTML = '<div style="text-align: center; padding: 2rem;"><h2>Failed to load.</h2><p>Please check your connection and refresh the page.</p></div>';
	}
};
