...
import { doc, getDoc, updateDoc, deleteField, arrayUnion, arrayRemove } from "https://www.gstatic.com/firebasejs/9.6.1/firebase-firestore.js";
...
const renderUserDashboard = () => {
    ...
    const videosCompleted = Object.values(userProgress.progress || {}).reduce((sum, p) => sum + p, 0);
    const totalVideos = Object.values(courseData || {}).reduce((sum, level) => sum + (level.totalVideos || 0), 0);
    const userQuizPoints = Object.values(userProgress.quizScores || {}).reduce((sum, result) => sum + result.score, 0);
    let totalQuizQuestions = 0;
    const canonicalQuizData = quizData['en'] || {};
    Object.values(canonicalQuizData).forEach(quiz => { totalQuizQuestions += quiz.length; });
...
const renderQuizzesView = () => {
    ...
    if (!container) return;
    const t = getTranslations();
    container.innerHTML = Object.keys(quizData.en).map((level, index) => `
...
const updateUIForGuest = () => {
    ...
    // Hide all modals that might be open from the authenticated view
...
const setupEventListeners = () => {
    ...
    document.body.addEventListener('click', async (e) => {
        ...
        } else if (navLink || (footerLink && footerLink.getAttribute('href').startsWith('#')) || headerProfileLink) {
            e.preventDefault();
            const link = navLink || footerLink || headerProfileLink;
